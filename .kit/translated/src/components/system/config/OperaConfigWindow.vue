...
export default {
  ...
  components: { ConfigInput <#noparse>}</#noparse>,
  extends: BaseOpera,
  data () {
    const _this = this
    return {
      ...
      rules: {
        code: [
          { required: true, message: '请输入配置编码' <#noparse>}</#noparse>
        ],
        name: [
          { required: true, message: '请输入配置名称' <#noparse>}</#noparse>
        ],
        value: [
          { required: true, message: '请输入配置值' <#noparse>}</#noparse>
        ],
        valueType: [
          { required: true, message: '请输入值类型' <#noparse>}</#noparse>
        ],
        scope: [
          { required: true, message: '请选择配置作用域' <#noparse>}</#noparse>
        ]
      <#noparse>}</#noparse>
    <#noparse>}</#noparse>
  <#noparse>}</#noparse>,
