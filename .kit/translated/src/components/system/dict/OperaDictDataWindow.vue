  ...
  >
    <el-form :model="form" ref="form" :rules="rules">
      ...
      <el-form-item label="状态" prop="disabled" required class="form-item-status">
        ...
        <span class="status-text">{{$filters.disabledText(form.disabled)<#noparse>}</#noparse><#noparse>}</#noparse></span>
      </el-form-item>
    </el-form>
  </GlobalWindow>
</template>
...
export default {
  ...
  data () {
    return {
      ...
      rules: {
        label: [
          { required: true, message: '请输入数据标签' <#noparse>}</#noparse>
        ],
        value: [
          { required: true, message: '请输入数据值' <#noparse>}</#noparse>
        ]
      <#noparse>}</#noparse>
    <#noparse>}</#noparse>
  <#noparse>}</#noparse>,
