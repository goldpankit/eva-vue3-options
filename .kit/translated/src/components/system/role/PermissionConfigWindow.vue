<template>
  ...
  >
    <div
      ...
      class="tip-wrap"
    >
      <p>为角色 <em>{{role.name<#noparse>}</#noparse><#noparse>}</#noparse></em> 配置权限</p>
      <el-alert
...
<script>
import { fetchPermissions, configPermissions <#noparse>}</#noparse> from '@/api/system/permission'
    ...
    confirm () {
      ...
      <#noparse>}</#noparse>)
        .then(() => {
          ...
          this.$emit('success')
          setTimeout(() => {
            this.$dialog.attentionConfirm('菜单权限配置成功，用户需重新登录后生效').then(() => {<#noparse>}</#noparse>)
          <#noparse>}</#noparse>, 300)
        <#noparse>}</#noparse>)
        .catch(e => {
          this.$tip.apiFailed(e)
