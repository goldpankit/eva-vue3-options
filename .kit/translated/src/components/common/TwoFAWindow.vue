<template>
  <el-dialog
    ...
    :show-close="false"
  >
    <slot name="beforeForm"><p class="two-fa-window__message">{{message<#noparse>}</#noparse><#noparse>}</#noparse></p></slot>
    ...
    <template #footer>
      <div class="two-fa-window__footer">
        ...
        <div class="opera">
          <el-button @click="cancel" :disabled="isWorking">{{cancelText<#noparse>}</#noparse><#noparse>}</#noparse></el-button>
...
export default {
  name: 'TwoFAWindow',
  emits: ['confirm'],
  components: { PasswordInput <#noparse>}</#noparse>,
  props: {
    // 标题
    title: {
      type: String,
  ...
  data () {
    return {
      ...
      // 表单验证规则
      rules: {
        password: { required: true, message: '请输入登录密码' <#noparse>}</#noparse>
      <#noparse>}</#noparse>
    <#noparse>}</#noparse>
  <#noparse>}</#noparse>,
  methods: {
