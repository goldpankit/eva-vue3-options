      ...
      <h2>
        ...
        <el-breadcrumb separator="/">
          <el-breadcrumb-item
            v-for="path in paths"
            :key="path"
          >{{path<#noparse>}</#noparse><#noparse>}</#noparse></el-breadcrumb-item>
        </el-breadcrumb>
      </h2>
      <div class="user">
        <el-dropdown trigger="click">
          <span class="el-dropdown-link">
            <img
              ...
              alt=""
            >
            {{displayName(userInfo)<#noparse>}</#noparse><#noparse>}</#noparse>
            <el-icon>
              <ArrowDown />
            </el-icon>
          </span>
...
import { logout, updatePwd <#noparse>}</#noparse> from '@/api/system'
...
export default {
  ...
  data () {
    return {
      ...
      changePwdData: {
        ...
        rules: {
          oldPwd: [
            { required: true, message: '请输入原始密码' <#noparse>}</#noparse>
          ],
          newPwd: [
            { required: true, message: '请输入新密码' <#noparse>}</#noparse>
          ],
          confirmPwd: [
            { required: true, message: '请再次输入新密码' <#noparse>}</#noparse>
          ]
        <#noparse>}</#noparse>
      <#noparse>}</#noparse>
    <#noparse>}</#noparse>
  ...
  methods: {
     ...
     * 展示名称
     *
     * @param {Object<#noparse>}</#noparse> userInfo 用户信息
     */
